...
_OWRT_VERSION_MAP = {
    ConnectionTag.VM_OPENWRT_GW_1: "24.10.4",
    ConnectionTag.VM_OPENWRT_GW_2: "25.12.0",
}


def _dist_path(tag: ConnectionTag) -> str:
    version = _OWRT_VERSION_MAP[tag]
    return f"dist/openwrt/{_PROFILE}/x86_64/{version}/"
...
def _opkg_cdn_files(tag: ConnectionTag) -> list[tuple[str, str]]:
    dist = _dist_path(tag)
    return [
        (f"{dist}{NORDVPNLITE_IPK_NAME}", CDN_FEED_DIR),
        (f"{dist}Packages", CDN_FEED_DIR),
        (f"{dist}Packages.gz", CDN_FEED_DIR),
        (f"{dist}Packages.sig", CDN_FEED_DIR),
        (f"{dist}{PUB_KEY_NAME_OPKG}", CDN_WEB_ROOT),
    ]


def _apk_cdn_files(tag: ConnectionTag) -> list[tuple[str, str]]:
    dist = _dist_path(tag)
    return [
        (f"{dist}{NORDVPNLITE_APK_NAME}", CDN_FEED_DIR),
        (f"{dist}packages.adb", CDN_FEED_DIR),
        (f"{dist}{PUB_KEY_NAME_APK}", CDN_WEB_ROOT),
    ]
...
async def copy_binaries(
    ...
    async with asyncssh.connect(
        ...
        if tag == ConnectionTag.VM_OPENWRT_GW_2:
            await copy_artifacts_to_cdn(cdn_ssh_connection, _apk_cdn_files(tag))
            await setup_openwrt_vm_apk(connection)
        else:
            await copy_artifacts_to_cdn(cdn_ssh_connection, _opkg_cdn_files(tag))
